...
module.exports.run = async (client, message, args, userInfo) => {
  if (userInfo && userInfo.class !== '') return message.channel.send(`:warning: **${message.author.username} Tu Possède déjà une Fiche D'Aventure !**`);

  const q = args.join(" ");
  const position = classes.map(e => e.name.toLowerCase()).indexOf(q.toLowerCase());

  if (q && position == -1) message.channel.send(`:warning: **${message.member} cette classe n'éxiste pas ! Veuillez choisir une classe valide !**`)


  if (q && position !== -1) {
    try {
      const classe = classes[position];
      message.channel.send(`**Voulez vous vraiment entrer dans le monde de Rasgart en temps que \`${classe.name}\`** (oui ou non)\n\n:warning: **Choississez bien votre classe car le choix de celle-ci est définitif !**`)
